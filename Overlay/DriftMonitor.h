...
//    tremor, weight shifts), so they cannot qualify; a plain variance gate
//    fired on every user who had just stopped moving.
...
//    displacement magnitude, so there is no calibration feedback loop.
...
public:
	...
	// Each device is evaluated independently. `linearScale` converts raw
	// tracking units into calibrated meters: 1 for reference-system samples,
	// the active calibration scale for target-system ones.
...
private:
	...
	// The last accepted sample, always written as a whole.
	struct LastValid
	{
